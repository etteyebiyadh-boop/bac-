...
export function HeroPathSelector() {
  ...
  return (
    <div className="stack" style={{ width: "100%", maxWidth: "900px", margin: "0 auto" }}>
      {step === 1 && (
        <div className="stack" style={{ gap: "32px", animation: "reveal-up 0.5s ease" }}>
          <h2 className="section-title" style={{ fontSize: "2.5rem" }}>Step 1: Choose Your Section.</h2>
      ...
      {step === 2 && (
        <div className="stack" style={{ gap: "32px", animation: "reveal-up 0.5s ease" }}>
          <div className="row-between" style={{ justifyContent: "flex-start", gap: "12px" }}>
            <button onClick={() => setStep(1)} className="pill" style={{ cursor: "pointer", background: "transparent" }}>← Back</button>
            <span className="pill" style={{ background: "var(--primary)", color: "white" }}>Section: {section.replace("_", " ")}</span>
          </div>
          <h2 className="section-title" style={{ fontSize: "2.5rem" }}>Step 2: Choose Your Optional Language.</h2>
          <div className="grid grid-cols-3" style={{ gap: "16px" }}>
            {LANGUAGES.map((l) => (
              <button 
                key={l.id} 
      ...
      {step === 3 && (
        <div className="stack" style={{ gap: "40px", animation: "reveal-up 0.5s ease" }}>
          <div className="row-between" style={{ justifyContent: "flex-start", gap: "12px" }}>
             <button onClick={() => setStep(2)} className="pill" style={{ cursor: "pointer", background: "transparent" }}>← Back</button>
          ...
          <div className="card stack" style={{ padding: "60px", background: "radial-gradient(circle at top right, rgba(99, 102, 241, 0.15), transparent)", border: "1px solid var(--primary)" }}>
             <span className="eyebrow" style={{ color: "var(--primary)" }}>The Excellence Path</span>
             <h2 className="section-title">Your Personalized Roadmap is Ready.</h2>
             <p className="muted" style={{ fontSize: "1.2rem", maxWidth: "600px" }}>
               We've generated the exact sequence of lessons and missions needed for a 17/20 in your track.
             </p>

             <div className="stack" style={{ marginTop: "40px", gap: "16px", textAlign: "left" }}>
                <div className="row-between" style={{ padding: "16px", background: "rgba(255,255,255,0.02)", borderRadius: "12px", border: "1px solid var(--glass-border)" }}>
                   <div className="row-between" style={{ gap: "16px" }}>
                      ...
                      <strong style={{ fontSize: "1rem" }}>Arabic: Analysis of Literary Critical Terms (Coef 2)</strong>
                   </div>
                   <span>⏳ 15 min</span>
                </div>
                <div className="row-between" style={{ padding: "16px", background: "rgba(255,255,255,0.02)", borderRadius: "12px", border: "1px solid var(--glass-border)" }}>
                   <div className="row-between" style={{ gap: "16px" }}>
                      ...
                      <strong style={{ fontSize: "1rem" }}>French: Mastering the Concordance of Tenses (Coef 2)</strong>
                   </div>
                   <span>⏳ 20 min</span>
                </div>
                <div className="row-between" style={{ padding: "16px", background: "rgba(255,255,255,0.02)", borderRadius: "12px", border: "1px solid var(--glass-border)" }}>
                   <div className="row-between" style={{ gap: "16px" }}>
                      ...
                      <strong style={{ fontSize: "1rem" }}>{optionalLang}: Interactive Language Immersion</strong>
                   </div>
                   <span>⏳ 25 min</span>
                </div>
             </div>
             ...
             <div className="row-between" style={{ marginTop: "60px", gap: "24px" }}>
                <Link className="button-link" href={`/auth/signup?section=${section}&lang=${optionalLang}`} style={{ flex: 1, justifyContent: "center", padding: "24px" }}>
                   Secure My Excellence Path 🚀
                </Link>
                <button onClick={reset} className="button-link button-secondary" style={{ padding: "24px" }}>Start Over</button>
             </div>
          </div>
        </div>
      )}
